...
const ushort OccGridRLE::HIGHEST_BIT = 0x8000;
float _temp_float;
int _temp_int;
unsigned short _temp_ushort;
...
OccGridRLE::write(char *filename)
{
	...
	for (yy = 0; yy < this->ydim; yy++) {
		...
		if (runType == OccGridRLE::CONSTANT_DATA) {
			...
			write_be_ushort(element, sizeof(OccElement), 1, fp);
                        using namespace std;
//                        cout << "cur_val: " << element->value << endl;
//                        cout << "cur_w: " << element->totalWeight << endl;
//                        cout << "cur_color: " << element->color << endl;
		    }
		}
	    }
	}
