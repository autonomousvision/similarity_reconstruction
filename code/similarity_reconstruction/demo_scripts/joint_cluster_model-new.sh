...
test_tsdf_align_bin=/home/dell/codebase/mpi_project/urban_reconstruction/code/build/hashmap/bin/test_joint_cluster_model
...
optimization_bin=/home/dell/codebase/mpi_project/urban_reconstruction/code/build_handin/bin/test_tsdf_optimization
scene_file=$input_model
detect_obb_file=$detect_box_txt
output_dir2=$output_root/refractor-joint-pca-$pca_num-lavgscale-$lambda_average_scale-lobs-$lambda_obs-lregrot-$lambda_reg_rot-lregscale-$lambda_reg_scale-lregtrans-$lambda_reg_trans-lzscale-$lambda_reg_zscale-0.01percentile-rpca-robustalign-test-onlyalignterm-ls/
if [ ! -d "$output_dir2" ]; then
    mkdir $output_dir2
fi
output_prefix=$output_dir2
#echo $optimization_bin --scene_file $scene_file --detect_obb_file $detect_obb_file --output_prefix $output_prefix
echo $optimization_bin --scene_file $scene_file --detect_obb_file $detect_obb_file --output_prefix $output_prefix --lambda_avg_scale $lambda_average_scale --lambda_obs $lambda_obs --lambda_reg $lambda_reg_rot --noise_observation_thresh $noise_counter_thresh --noise_connected_component_thresh $noise_comp_thresh
sleep 2
$optimization_bin --scene_file $scene_file --detect_obb_file $detect_obb_file --output_prefix $output_prefix --lambda_avg_scale $lambda_average_scale --lambda_obs $lambda_obs --lambda_reg $lambda_reg_rot --noise_observation_thresh $noise_counter_thresh --noise_connected_component_thresh $noise_comp_thresh
