...
// defaults
static int rampSize = 5;
static double resolution = 0.002;
static bool hardEdges = true;
static bool useConf = false;
static int stripEdges = 0;
static int algorithm = 0;
static int exact_ramp_size = 0;
static OccGridRLE *templateVRI = NULL;
...
// scan convert a triangle into the voxel grid
	...
	for (i=0; i < 3; i++) {
        bb[i*2+0] = dMax(0, ceil(triBB[i*2+0]) - exact_ramp_size);
        bb[i*2+1] = dMin(vg.max[i]-1, floor(triBB[i*2+1]) + exact_ramp_size);
	...
	for (z = bb[4]; z <= bb[5]; z++) {
		for (y = bb[2]; y <= bb[3]; y++) {
			for (x = bb[0]; x <= bb[1]; x++) {
				...
				tm.rayOrig = voxelPt;
                tm.closestDist = dMin(exact_ramp_size, fabs(((double)vg.dist(x, y, z) / USHRT_MAX) - 0.5) * (rampSize * 2));
...
static void check_neighbour(TriMesh& tm, VoxelGrid& vg, int i0, int j0, int k0, int i1, int j1, int k1)
{
    if(vg.closest_tri_idx(i1,j1,k1)>=0 && vg.conf(i1, j1, k1) > 0)
    {
        int tri = vg.closest_tri_idx(i1,j1,k1) * 3;
        Vec3d voxelPt(i0, j0, k0);

        tm.rayOrig = voxelPt;
        float current_dist_abs = fabs(((double)vg.dist(i0, j0, k0) / USHRT_MAX) - 0.5) * (rampSize * 2);
        if (vg.conf(i0, j0, k0) > 0 && current_dist_abs <= exact_ramp_size) return;
        tm.closestDist = dMin(rampSize, current_dist_abs);
...
static void sweep(TriMesh& tm, VoxelGrid& vg, int di, int dj, int dk)
{
   int i0, i1;
   if(di>0){ i0=1; i1=vg.max[0]; }
   else{ i0=vg.max[0]-2; i1=-1; }
   int j0, j1;
   if(dj>0){ j0=1; j1=vg.max[1]; }
   else{ j0=vg.max[1]-2; j1=-1; }
   int k0, k1;
   if(dk>0){ k0=1; k1=vg.max[2]; }
   else{ k0=vg.max[2]-2; k1=-1; }
   for(int k=k0; k!=k1; k+=dk) for(int j=j0; j!=j1; j+=dj) for(int i=i0; i!=i1; i+=di) {
      check_neighbour(tm, vg, i, j, k, i-di, j,    k);
      check_neighbour(tm, vg, i, j, k, i,    j-dj, k);
      check_neighbour(tm, vg, i, j, k, i-di, j-dj, k);
      check_neighbour(tm, vg, i, j, k, i,    j,    k-dk);
      check_neighbour(tm, vg, i, j, k, i-di, j,    k-dk);
      check_neighbour(tm, vg, i, j, k, i,    j-dj, k-dk);
      check_neighbour(tm, vg, i, j, k, i-di, j-dj, k-dk);
   }
}

static void sweepTriangles(TriMesh& tm, VoxelGrid& vg);
static void processTriangleInexact(TriMesh &tm, int tri, VoxelGrid &vg);
static void sweepBBox(TriMesh& tm, VoxelGrid& vg, int di, int dj, int dk, Vec3i boxmin, Vec3i boxmax);
static void sweepTriangles(TriMesh& tm, VoxelGrid& vg)
{
    int tenPercent;
    tenPercent = (tm.m_tris.size() / 3 / 10) * 3;
    cout << "scan converting inexact..." << endl << " |          |\n  ";
    for (int i=0; i < tm.m_tris.size(); i += 3) {
        if (tenPercent > 0 && i % tenPercent == 0) {
            cout << "*";
            cout.flush();
        }
        processTriangleInexact(tm, i, vg);
    }
}
...
// scan convert a triangle into the voxel grid
    ...
    Vec3i boxmin(bb[0], bb[2], bb[4]);
    Vec3i boxmax(bb[1], bb[3], bb[5]);

    for(unsigned int pass=0; pass<2; ++pass) {
        sweepBBox(tm, vg, +1, +1, +1, boxmin, boxmax);
        sweepBBox(tm, vg, -1, -1, -1, boxmin, boxmax);
        sweepBBox(tm, vg, +1, +1, -1, boxmin, boxmax);
        sweepBBox(tm, vg, -1, -1, +1, boxmin, boxmax);
        sweepBBox(tm, vg, +1, -1, +1, boxmin, boxmax);
...
{
   int i0, i1;
   if(di>0){ i0=boxmin[0] + 1; i1=boxmax[0] + 1; }
   else{ i0=boxmax[0]-1; i1=boxmin[0]-1; }
   int j0, j1;
   if(dj>0){ j0=boxmin[1] + 1; j1=boxmax[1] + 1; }
   else{ j0=boxmax[1]-1; j1=boxmin[1]-1; }
   int k0, k1;
   if(dk>0){ k0=boxmin[2] + 1; k1=boxmax[2] + 1; }
   else{ k0=boxmax[2]-1; k1=boxmin[2]-1; }
   for(int k=k0; k!=k1; k+=dk) for(int j=j0; j!=j1; j+=dj) for(int i=i0; i!=i1; i+=di){
      check_neighbour(tm, vg, i, j, k, i-di, j,    k);
      check_neighbour(tm, vg, i, j, k, i,    j-dj, k);
      check_neighbour(tm, vg, i, j, k, i-di, j-dj, k);
...
int ReadPLYAndConvertToVRI(const PLY2VRIOptions& options)
{
    ...
    rampSize = options.rampSize;
    hardEdges = options.hardEdges;
    stripEdges = options.stripEdges;
    algorithm = options.algorithm;
    exact_ramp_size = options.exact_ramp_size;
    strcpy(input, options.input_filename.c_str());
    strcpy(output, options.output_filename.c_str());
   // input = options.input_filename;
...
//	}
    ...
    case 0:
    {
        ...
        cout << endl;
        // added by zc, fast sweeping method
        if (rampSize > exact_ramp_size) {
            //sweepTriangles(tm, vg);
            //cout << endl;
            for(unsigned int pass=0; pass<2; ++pass) {
                // sweep: input: VoxelGrid: vg; Trimesh: voxel coord triangles; dx: 1 in this case
                // static void sweep(TriMesh& tm, VoxelGrid& vg, int di, int dj, int dk)
                sweep(tm, vg, +1, +1, +1);
                sweep(tm, vg, -1, -1, -1);
                sweep(tm, vg, +1, +1, -1);
