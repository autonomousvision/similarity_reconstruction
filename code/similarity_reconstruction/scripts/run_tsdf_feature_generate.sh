...
for vlen in 0.5 1 2
do
output_prefix=/home/dell/codebase/mpi_project/urban_reconstruction/code/build/hashmap/results/tsdf_feature_test_600frame_n1_onesample_jittering_$vlen
if [ ! -d "$output_prefix" ]; then
mkdir $output_prefix
fi
output_prefix=$output_prefix"/house_1"
echo "$test_bin $input_samples --in-model $input_model --out-dir-prefix $output_prefix --save_tsdf_bin --sample_num 3 --mesh-min-weight 0.5 --voxel_length $vlen"

$test_bin $input_samples --in-model $input_model --out-dir-prefix $output_prefix --save_tsdf_bin --sample_num 2000 --mesh-min-weight 0.2 --voxel_length $vlen
done
