...
data_root=/home/dell/Data/test1/image_03/
...
outputdir=$output_root"output-3d-model-camera23-ntest1-cam3/"
mkdir $outputdir
set -e
...
$recon_bin --in-root $data_root --out $outputfile_conf".ply" --save-ascii --startimage $startimg --endimage $endimg --flatten --min_weight $weight --diffusion-smooth --niteration $niteration --truncation_limit $maxdist --voxel_length $voxellen --do_fuse --param-prefix $param_prefix --image-prefix $image_prefix --depth-prefix $depth_prefix --cleanup --help
done
done
done
