...
output_dir=$adjust_output_root/adjusted_obbs_score_distribution
...
out_dir_prefix=$output_dir/adjusted2_obbs.txt
...
if [ $do_adjust_obbs -gt 0 ]; then
    ...
    sleep 2
    $adjust_bin --in-model $in_model --detected-obb-file $detected_obb_file --detected-obb-score-file $detected_obb_scores --sample_voxel_sidelengths $sample_voxel_sidelengths --out-dir-prefix $out_dir_prefix --svm_model $svm_model
fi
