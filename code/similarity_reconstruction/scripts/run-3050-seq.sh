...
run_root=$result_root/seperate-seq-batch-all-seq2-detector-building2-car-try3-rebut-n1/
...
lowest_score_to_supress_arr=(-0.5 -0.5 -0.50)
noise_counter_thresh_arr=(1 1 3)
...
merge_score_thresh_arr=(-0.26 0.3 0.5)
do_train_detector_arr=(0 0 0)
do_detection_arr=(0 0 0)
do_NMS_arr=(0 0 0)
do_pr_arr=(0 0 0)
do_adjust_obbs_arr=(0 0 0)
...
##
    ...
    skymap_check=1
    filter_noise=60
    consistency_check_root=$joint_learn_output_root/init_consistency_check-$joint_learn_suffix-$st_neighbor-$ed_neighbor-pcanum-$pcanum
    ...
    skymap_check=1
    filter_noise=120
    consistency_check_root=$joint_learn_output_root/consistency_check-aftjointlearn-$joint_learn_suffix-$st_neighbor-$ed_neighbor-pcanum-$pcanum
    echo consistency_check_root $consistency_check_root
    if [ ! -d $consistency_check_root ]; then
        mkdir $consistency_check_root
    fi
...
#in_models_merge=${consistent_tsdf_output_category_arr[@]}
#out_dir=$run_root/batch-seq2-res-$startimg-$endimg-multi_cate_merged-pcanum-$pcanum
#if [ ! -d $out_dir ]; then
#    mkdir $out_dir
#fi
#out_prefix=$out_dir/out_sample
