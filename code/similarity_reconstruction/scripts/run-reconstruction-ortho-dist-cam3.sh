...
output_root=$result_root/reconstruction_closest_debug_saveppm_clampgrid_whole1
...
output_suffix=$cam_suffix"-st-$startimg-ed-$endimg-vlen-$voxel_length-rampsz-$ramp_size-try1"-s$strip_triangles
...
dep2ply_out_dir=$output_root/"output-depthmap-meshing-ddfact-$dd_factor-edgeratio-$edge_ratio-margin-$margin"$output_suffix  # output dir
...
#output_vri_suffix=.rampsz-"$ramp_size".vlen-"$voxel_length".ppm
if [ $do_ply2vri -gt 0 ]; then
  ...
  do
   ...
   echo $ply2vri_path -r$voxel_length -l$ramp_size -h1 -s$strip_triangles $input_ply_path $output1
   #echo $ply2vri_path -r$voxel_length -l$ramp_size -h1 -s0 $input_ply_path $output1
   #job_pool_run nice -15 $ply2vri_path -r$voxel_length -l$ramp_size -h1 $input_ply_path $output1
   $ply2vri_path -r$voxel_length -l$ramp_size -h1 -s$strip_triangles $input_ply_path $output1
