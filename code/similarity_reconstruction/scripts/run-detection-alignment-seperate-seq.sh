...
vz=6
#
#train_detect_delta_x=0.5
#train_detect_delta_y=0.5
#train_detect_delta_rot=2.5 # degree
#
#detect_delta_x=0.5
#detect_delta_y=0.5
#detect_delta_rot=2.5

train_detect_delta_x=10
train_detect_delta_y=10
train_detect_delta_rot=30
...
do
    ...
    if [ $do_merge_tsdf_obb -gt 0 ]; then
        echo $merge_bin --in-models $test_input_tsdf --in-obbs $detect_obb_infos --out $merge_output_prefix --obb-min-score 0
        $merge_bin --in-models $test_input_tsdf --in-obbs $detect_obb_infos --out $merge_output_prefix --obb-min-score 0
    fi
    merged_model=$merge_output_prefix"_tsdf.bin"
    merged_detect_box_txt=$merge_output_prefix".obb_infos.txt"
    
    echo "################## joint learn ###################"
    joint_learn_output_root=$batch_output_root/joint_learn-try2-$seq_i
