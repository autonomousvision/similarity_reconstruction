...
void SaveRenderedImage(int width, int height, int color)
{
    ...
    cv::Mat im_mat(height, width, CV_8UC3, (unsigned char*)imageData);
    cv::cvtColor(im_mat, im_mat, CV_RGB2BGR);
    cv::imwrite("test1.png", im_mat);
...
*/
// modified from http://paulbourke.net/miscellaneous/windowdump/
// added by zc
bool WindowDump(const std::string& file_path = std::string(), bool add_counter = true)
{
   const int width = glutGet(GLUT_WINDOW_WIDTH);
   const int height = glutGet(GLUT_WINDOW_HEIGHT);
   static int counter = 0; /* This supports animation sequences */
   std::unique_ptr<unsigned char []> image(new unsigned char[3 * width * height]);

   glPixelStorei(GL_PACK_ALIGNMENT,1);
   /* Copy the image into our buffer */
   // glReadBuffer(GL_BACK);
   glReadPixels(0,0,width,height,GL_RGB,GL_UNSIGNED_BYTE,image.get());
   cv::Mat im_mat(height, width, CV_8UC3, image.get());
   cv::cvtColor(im_mat, im_mat, CV_RGB2BGR);
   cv::flip(im_mat, im_mat, 0);
   char fsuffix[32] = {'\0'};
   if (add_counter)
   {
       sprintf(fsuffix, "%04d", counter);
   }
   string output_filename = file_path + "_" + fsuffix + ".png";
   cv::imwrite(output_filename, im_mat);
...
void setup_lighting(int id)
{
    ...
    GLfloat global_ambient[] = { 0.22f, 0.22f, 0.25f, 0.25f };
...
void resetview()
{
    ...
    // load mesh xform from command lines
    for (size_t i = 0; i < meshes.size(); i++)
        if (!xform_names[i].empty())
            xforms[i].read(xform_names[i]);
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (!strcmp(argv[i], "-presetcamxf")) {
            if (!preset_xf.read(argv[++i]))
            {
                std::cerr << "reading preset camera xf file failed. " << std::endl;
                exit(EXIT_FAILURE);
            }
            is_xf_preset = true;
            continue;
        }
        if (strlen(argv[i]) > 3 && std::string(argv[i]).substr(strlen(argv[i]) - 3, 3) == ".xf") {
            xform_names.back() = std::string(argv[i]);
            continue;
        }
        ...
        xform_names.push_back(std::string());
        visible.push_back(true);
        filenames.push_back(filename);
    }

    //////////////////////
    // Create a window.  (must) //
    //////////////////////
    // glutInit( &argc, argv );
    ...
    case GL_FRAMEBUFFER_UNSUPPORTED_EXT:
        cerr<<"GL_FRAMEBUFFER_UNSUPPORTED_EXT!: ERROR"<<endl;
        exit(EXIT_FAILURE);
        break;
    default:
        cerr<<"GL_FRAMEBUFFER_COMPLETE_EXT!: UNKNOWN ERROR"<<endl;
        exit(EXIT_FAILURE);
    }
    ...
    cv::Mat im_mat(height, width, CV_8UC3, color);
    cv::cvtColor(im_mat, im_mat, CV_RGB2BGR);
    cv::flip(im_mat, im_mat, 0);
    cv::imwrite(output_file, im_mat);
    delete[] color;
...
//    Below: a method that may work, too
//    glutCreateWindow( "RTT window" );
//    glewInit();
//    // prepare opengl environment
//    GLuint color;
//    GLuint depth;
//    GLuint fbo;
...
//    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, width, height, 0, GL_RGB, GL_UNSIGNED_BYTE, NULL);
//    glBindTexture(GL_TEXTURE_2D, 0);
//    fprintf(stderr, "prepared opengl texture obj\n");
//    glGenRenderbuffers(1, &depth);
//    glBindRenderbuffer(GL_RENDERBUFFER, depth);
//    glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT, width, height);
//    glBindRenderbuffer(GL_RENDERBUFFER, 0);
//    fprintf(stderr, "prepared opengl renderbuffer obj\n");
//    glGenFramebuffers(1, &fbo);
//    glBindFramebuffer(GL_FRAMEBUFFER, fbo);
//    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_2D, color, 0);
//    glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, depth);
//    glBindFramebuffer(GL_FRAMEBUFFER, 0);
//    fprintf(stderr, "prepared opengl framebuffer obj\n");
//    if (offscreen)
//        glBindFramebuffer(GL_FRAMEBUFFER, fbo);
//    else
//        glBindFramebuffer(GL_FRAMEBUFFER, 0);
...
//    resetview();
//    redraw();
//    fprintf(stderr, "redraw finished\n");
//    if (offscreen)
//       SaveRenderedImage(width, height, color);
}
