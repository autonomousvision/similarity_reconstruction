...
void RectifiedCameraPair::InitializeBackProjectionBuffers(int im_width, int im_height) {
    if (im_width_ == im_width && im_height_ == im_height && !cos_phis_.empty() && delta_theta_inv_ > 0) {
        fprintf(stderr, "\nout of initial buffer\n");
        return;
    }
    ...
    sin_thetas_.resize(im_width);
    cos_thetas_.resize(im_width);
    sin_phis_.resize(im_height);
    cos_phis_.resize(im_height);
...
//     cout << "debug: rotation" << (cv::Mat)rotation <<endl;
//     cout << "debug: translation" << (cv::Mat)translation <<endl;
